    ...
    <div class="w-full max-w-7xl">
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 md:justify-between bg-gray-50 shadow-lg grid-col p-2 border-b md:rounded-t-3xl">
            ...
            <div class="flex flex-row gap-3">
                <a href="{{route('ventas.create')}}" class="flex justify-center w-full bg-gradient-to-r from-green-300 to-green-500 text-white text-xl hover:from-green-600 hover:to-green-600 focus:outline-none  p-2 rounded-full hover:bg-green-600 md:w-72">
                    Ingresar venta
                </a>
        ...
        <form method='post' action="{{route('ventas.add')}}">
            ...
            <div class="bg-white shadow-lg flex flex-col space-y-3 pt-3 px-3 md:space-y-0 md:flex-row md:justify-between">
                <div class="flex flex-col space-y-3 md:flex-row md:space-y-0">
                    ...
                    <input required class="hidden" name="id" value="{{Session::get('buscaPro')->id}}">
                    <input required type="text" name="nombre" value="{{Session::get('buscaPro')->nombre}}" placeholder="Nombre item" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:rounded-l-full md:w-40" disabled>
                    <input required type="text" name="codigo" value="{{Session::get('buscaPro')->codigo}}" placeholder="Codigo item" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:w-40" disabled>
                    <input required min="1" required type="number" name="cantidad" value="1" min="1" placeholder="Ingresar Cantidad" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:rounded-r-full md:w-44">
                    @else

                    <input required type="text" name="nombre" placeholder="Nombre item" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:rounded-l-full md:w-40" disabled>
                    <input required type="text" name="codigo" placeholder="Codigo item" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:w-40" disabled>
