...
<div class="flex flex-col md:p-5 items-center">
    <div class="w-full max-w-7xl">
        ...
        <form method='get' action="{{route('transacciones.search')}}">
            ...
            {{-- Fin Formulario de el buscador de items  --}}
            <div class="flex flex-row gap-2">
                <p class="w-24 placeholder-gray-400 px-6 py-3 border border-gray-300 text-center md:rounded-l-full">Total</p>
                <input type="text" value="{{\Session::get('totalcompra')}}" placeholder="Total Compra" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 text-right md:rounded-r-full md:w-40" disabled>
            </div>
        </div>
        {{-- Formulario que muestra el resultado de buscar item --}}
        <form method='post' action="{{route('transacciones.add')}}">
            @csrf
            <div class="bg-white shadow-lg flex flex-col space-y-3 pt-3 px-3 md:space-y-0 md:flex-row md:justify-between">
                <div class="flex flex-col space-y-3 md:flex-row md:space-y-0">
                    @if(Session::has('buscaPros'))
                    <input class="hidden" name="id" value="{{Session::get('buscaPros')->id}}">
                    <input type="text" name="nombre" value="{{Session::get('buscaPros')->nombre}}" placeholder="Nombre item" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:rounded-l-full md:w-40" disabled>
                    <input type="text" name="codigo" value="{{Session::get('buscaPros')->codigo}}" placeholder="Codigo item" class="w-full placeholder-gray-400 px-6 py-3 border border-gray-300 md:w-40" disabled>
        ...
        <table class="w-full">
            ...
            <tbody>
                <tr class="grid grid-cols-6 justify-items-center border rounded-b-3xl">
                    ...
                    <td>
                        <form action="{{route('transacciones.del')}}" method="post">
                            @csrf
